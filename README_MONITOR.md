...
- **dns_monitor.py**: Monitor de eventos DNS para milnomes.es
...
Monitorea consultas DNS relacionadas con milnomes.es:
```bash
sudo python3 dns_monitor.py
```

**Características del monitor DNS**:
- Captura consultas DNS en tiempo real
- Detecta DNS tunneling y exfiltración
- Identifica subdomain enumeration
- Detecta intentos de DNS amplification
- Analiza posibles DNS cache poisoning
- Registra todas las consultas y respuestas DNS
...
#### Monitor DNS (dns_monitor.py)
- Captura de consultas DNS en tiempo real (puerto 53)
- Detección de DNS tunneling y exfiltración
- Identificación de subdomain enumeration
- Detección de intentos de DNS amplification
- Análisis de posibles DNS cache poisoning
- Estadísticas de tipos de consulta DNS (A, AAAA, MX, TXT, etc.)
- Registro de IPs origen y respuestas DNS

## Archivos de Log

Los eventos se guardan en archivos separados según el tipo de monitoreo:

- **web_attacks_log.txt**: Eventos y ataques web (scripts web_monitor*.py y analyze_web_logs.py)
- **dns_events_log.txt**: Eventos y consultas DNS (script dns_monitor.py)

Ambos archivos se crean en la misma carpeta del script cuando se ejecutan.
...
├── dns_monitor.py              # Monitor de eventos DNS
├── README_MONITOR.md           # Esta documentación
├── INSTRUCCIONES_EJECUCION.md  # Instrucciones de ejecución
├── web_attacks_log.txt         # Archivo de log web (se crea al ejecutar)
└── dns_events_log.txt          # Archivo de log DNS (se crea al ejecutar)
```
